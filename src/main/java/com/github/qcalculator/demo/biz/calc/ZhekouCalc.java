...
public class ZhekouCalc extends AbstractCalculator<GoodsItem> {
  ...
  @Override
  public long calc(DiscountContext<GoodsItem> context, DiscountWrapper discountWrapper,
      Map<Long, GoodsItem> records, long prevStagePrice, CalcStage curStage) {
    List<GoodsItem> items = Lists.newArrayList(
        context.getDiscountItemGroup().get(discountWrapper.getId()));
    Map<Long, List<GoodsItem>> map = (Map<Long, List<GoodsItem>>) context.getPreCompute()
        .get("GroupBySkuIdPreCompute");
    long maxPrice = 0L;
    Collection<GoodsItem> group = Collections.emptyList();
