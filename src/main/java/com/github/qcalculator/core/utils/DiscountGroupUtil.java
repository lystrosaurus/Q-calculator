...
public class DiscountGroupUtil {
  ...
  private DiscountGroupUtil() {
  }

  /**
   * 返回所有共享组 Pair的left是长度不超过 Permutation.SUPPORTEDSIZE 的内容，right是第 Permutation.SUPPORTEDSIZE+1
   ...
   * @return
   */
  public static List<Pair<Set<DiscountWrapper>, Set<DiscountWrapper>>> transform(
      List<List<DiscountGroup>> groups, Map<String, Map<String, DiscountWrapper>> inMap) {
    ...
    for (List<Item> items : list) {
      ...
      if (CollUtil.isNotEmpty(discountWrappers) && discountWrappers.size() >= 2) {
    ...
    return resultList.stream()
        .sorted(Comparator.comparing(x -> x.getLeft().size(), Collections.reverseOrder()))
   ...
   */
  private static Pair<Set<DiscountWrapper>, Set<DiscountWrapper>> balanceLR(
      Set<DiscountWrapper> discountWrappers) {
   ...
   * @param inMap  实际可用优惠
   */
  private static List<List<Item>> mergeGroups(List<List<DiscountGroup>> groups,
      Map<String, Map<String, DiscountWrapper>> inMap) {
    if (CollUtil.isEmpty(groups) || CollUtil.isEmpty(inMap)) {
      return null;
    }
    //resultList 接收最终的结果
    ...
    return resultList.stream().filter(CollUtil::isNotEmpty).collect(Collectors.toList());
  }
