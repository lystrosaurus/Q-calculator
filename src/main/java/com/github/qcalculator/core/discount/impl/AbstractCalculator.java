...
public abstract class AbstractCalculator<T extends GoodsItem> implements Calculator<T> {
  ...
  public long calcWrap(DiscountContext<T> context, DiscountWrapper discountWrapper,
      Map<Long, T> records, byte idx, int i) {
   ...
   * @return
   */
  public abstract long calc(DiscountContext<T> context, DiscountWrapper discountWrapper,
      Map<Long, T> records, long prevStagePrice, CalcStage curStage);
