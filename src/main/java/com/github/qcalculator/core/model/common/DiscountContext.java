...
public class DiscountContext<T extends GoodsItem> implements Serializable {
   ...
   * @return
   */
  public static <T extends GoodsItem> DiscountContext<T> create(long originalPrice,
      List<T> goodsItems, List<DiscountWrapper> discountWrappers) {
   ...
   */
  private static <T extends GoodsItem> void initDiscount(DiscountContext<T> c,
    ...
    if (GoodsType.ALL.getCode() == conf.getGoodsType()) {
      ...
      if (ItemIdType.ITEM.getCode() == conf.getItemIdType()) {
        ...
        list = list.stream().filter(x -> !CollUtil.intersection(
            conf.getItemIds(), x.getCategoryIds()).isEmpty()).sorted().collect(Collectors.toList());
      }
    } else {
      //指定不参与
      if (ItemIdType.ITEM.getCode() == conf.getItemIdType()) {
  ...
  private static <T extends GoodsItem> void runPreCompute(DiscountContext<T> c,
      DiscountWrapper discount, List<T> list) {
