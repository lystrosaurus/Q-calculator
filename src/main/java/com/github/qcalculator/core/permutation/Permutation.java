...
public abstract class Permutation<T extends GoodsItem> {
  ...
  private boolean calcInner(Calculator<T> calculator, DiscountWrapper wrapper, List<Byte> a,
      int i) {
    ...
    if (i < a.size() - 1) {
      ...
      int nextOrder = context.getDiscountWrappers().get(a.get(i + 1)).getDiscountConfig()
          .getCalculateGroup();
      if (order > nextOrder) {
        //优先级不符合则跳出
        return false;
  ...
  public boolean executeCalc(DiscountContext<T> context, List<Byte> a) {
    ...
    return curPrice < context.getCalcResult().getFinalPrice() && (CollUtil.isEmpty(this.mustUseSet)
        || isMustUse(stages, this.mustUseSet)) ||
        //相同最优解处理逻辑
        curPrice == context.getCalcResult().getFinalPrice() && (CollUtil.isEmpty(this.mustUseSet)
            || isMustUse(stages, this.mustUseSet)) && sameOptimumCondition(stages);
  }
  ...
  private boolean checkIfWakeUpJump(DiscountWrapper cachedWrap, DiscountWrapper wrapper) {
    return cachedWrap.getDiscountConfig().getCalculateGroup() > wrapper.getDiscountConfig()
        .getCalculateGroup();
  }
